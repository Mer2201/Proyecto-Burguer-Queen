...
export class PayPage {
  ...
  switch(this.optionAddress){
    case 'address-default':
      this.showNewAddress = false;
      const user = this.userOrderService.getUser();
      this.address = user?.address ?? ''; // ← Previene el error
      break;
    case 'choose-address':
    ...
    payWithStripe(){
      ...
      const paymentIntent: CreatePaymentIntent = {
        ...
        currency: 'EUR',
        customer_id: 'cus_SL3HTTHC6AgWRT'
      };
    ...
    detectChangesPayment(){
     const sub =  this.payment$.subscribe({
        next: () => {
          const payment = this.store.selectSnapshot
          (StripeState.payment);
          if(payment){
