...
import { KEY_ORDER } from '../constants/constant';
...
export class UserOrderService {
  ...
  async saveOrder(){
    await Preferences.set({ key: KEY_ORDER, value: JSON.stringify
    (this.order) });
  }
  ...
  async removeProduct(product: Product){
    remove(this.order.products, p => isEqual(p.product,
      product));
       await this.saveOrder();
  ...
  totalPrice(quantityProduct: QuantityProduct){
    ...
    const total = this.priceProduct(quantityProduct.product)
